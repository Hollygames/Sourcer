...
class Item : virtual public Thing, public ItemAttributes
{
	public:
		...
		bool canBeMovedByPlayer(Player* player);
		static std::string getAttributesDescription(const ItemType& it, const Item* item,
			bool& begin, bool isWeapon);
...
inline int32_t Item::getUniqueOwner() const
{
	bool ok;
...
inline std::string Item::getDescriptionText() const
{
	bool ok;
	std::string v = getStringAttribute("description", ok);
	if (ok) {
	...
	return Item::items[id].description;
}
#endif

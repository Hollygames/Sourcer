		...
		&& item->isStackable() && item->getParent() != this)
	{
		...
		for(ItemList::reverse_iterator cit = itemlist.rbegin(); cit != itemlist.rend(); ++cit, --n)
		{
			if((*cit)->getID() == item->getID() && (*cit)->getItemCount() < 100
				&& (*cit)->getUniqueOwner() == uniqueOwner)
			{
				*destItem = (*cit);
				index = n;
				return this;
...
bool Container::hasItemWithUniqueOwner() const
{
...
bool Container::canMove(Player* player) const
{
	uint32_t playerGuid = player->getGUID();
	for (Item* item : itemlist) {
		uint32_t uniqueOwnerId = item->getUniqueOwner();
		if (uniqueOwnerId != 0 && uniqueOwnerId != playerGuid) {
			return false;
		}
	}
	return true;

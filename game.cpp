	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint16_t count)
{
	...
	bool fromPlayer = (dynamic_cast<Player*>(item->getTopParent()) != nullptr);
	bool toPlayer;
	bool toDepot;
	if (Item* toItem = toCylinder->getItem()) {
		Cylinder* topItemParent = toItem->getTopParent();
		toPlayer = (dynamic_cast<Player*>(topItemParent) != nullptr);
		toDepot = (dynamic_cast<Depot*>(topItemParent) != nullptr);
	} else {
		toPlayer = (dynamic_cast<Player*>(toCylinder) != nullptr);
		toDepot = (dynamic_cast<Depot*>(toCylinder) != nullptr);
	}

	if (!player->hasCustomFlag(PlayerCustomFlag_CanPushAllItems) && !toDepot) {
		Container* container = item->getContainer();
		if ((!toPlayer && (item->getUniqueOwner() != 0 ||
			(container && container->hasItemWithUniqueOwner()))) ||
			(toPlayer && (!item->canBeMovedByPlayer(player) ||
			(container && !container->canMove(player))))) {
			player->sendCancel("You cannot throw a unique item.");
			return false;
		}
	}
	...
	{
		if(!(*it)->executeThrow(player, item, fromPos, toPos, fromPlayer, toPlayer) && !deny)
			deny = true;
	}
	...
	int32_t index, Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/)
{
	...
	{
		if (uniqueOwner != 0 || toItem->getUniqueOwner() != 0) {
			return RET_CANNOTEXCHANGEUNIQUE;
		}
	...
	if (stackable && toItem && toItem->getID() == itemId && toItem->getUniqueOwner() != uniqueOwner) {
		return RET_CANNOTSTACKUNIQUE;
	}
	...
	{
		uint8_t n = 0;
		// uint8_t n = 0; -- CORIRIJA SE TIVER DANDO ERRO
	...
	uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	if (tradeItem->getUniqueOwner() != 0 ||
		(tradeItem->getContainer() && tradeItem->getContainer()->hasItemWithUniqueOwner())) {
		player->sendCancel("You cannot trade a unique item.");
		return false;
	}

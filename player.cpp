...
void Player::dropLoot(Container* corpse)
{
	...
	if (Depot* depot = getDepot(getTown(), true)) {
		...
		const auto& corpseItems = corpse->getItemList();
		for(auto it = corpseItems.rbegin(), end = corpseItems.rend(); it != end; ++it) {
			if ((*it)->getUniqueOwner() != 0) {
...
{
    int32_t absorbPercent = std::min(it.abilities->absorb[combatType], static_cast<int16_t>(40)); // Limite de 40% para absorção
    blocked += (int32_t)std::ceil((double)(damage * absorbPercent) / 100.);
    if(item->hasCharges())
        transform = true;
}
        ...
        defenseReduction = std::min(defenseReduction, 40); // Limite de 40% para redução de defesa
        damage -= defenseReduction;
        blocked += defenseReduction;

        if(field && it.abilities->fieldAbsorb[combatType])
        {
            int32_t fieldAbsorbPercent = std::min(it.abilities->fieldAbsorb[combatType], static_cast<int16_t>(40)); // Limite de 40% para absorção de campo
            blocked += (int32_t)std::ceil((double)(damage * fieldAbsorbPercent) / 100.);
            if(item->hasCharges())
                transform = true;
        }
    ...
    if(outfitAttributes)
{
    ...
    if(tmp)
    {
        int32_t outfitAbsorbPercent = std::min(tmp, static_cast<uint32_t>(40)); // Limite de 40% para absorção de outfit
        blocked += (int32_t)std::ceil((double)(damage * outfitAbsorbPercent) / 100.);
    }
    ...
    {
        int32_t vocationAbsorbPercent = std::min(vocation->getAbsorb(combatType), static_cast<short>(40)); // Limite de 40% para absorção de vocação
        blocked += (int32_t)std::ceil((double)(damage * vocationAbsorbPercent) / 100.);
    }
	...
	uint32_t& flags)
{
	if(!index /*drop to capacity window*/ || index == INDEX_WHEREEVER)
	{
		*destItem = NULL;
		...
		int32_t uniqueOwner = item->getUniqueOwner();
		for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
		{
			if(Item* invItem = inventory[i])
			{
				...
				if(autoStack && item->isStackable() && __queryAdd(i, item, item->getItemCount(), 0)
					...
					&& invItem->getUniqueOwner() == uniqueOwner)
				{
					*destItem = invItem;
					index = i;
					return this;
...
void Player::receiveMarketItem(uint16_t index)
{
	...
	if (Item* item = container->getItem(index)) {
		...
		if (item->getUniqueOwner() != 0) {
			sendFYIBox("Voce nao pode anunciar item unico.");
			return;
		}
...
bool Player::cancelMarketOffer(uint64_t numeration)
{
    ...
    // Consultando a oferta do mercado do jogador
    query.str("");
    query << "SELECT `id`, `item_id`, `amount` FROM `players_mymarketoffers` WHERE `player_id` = " << getGUID() << " AND `transaction_id` = " << numeration << ";";
    if ((result = db->storeQuery(query.str())))
    {
        uint16_t item_id = result->getDataInt("item_id");
        uint16_t amount = result->getDataInt("amount");

        // Criando o item a partir do ID e da quantidade
        if (Item* item = Item::CreateItem(item_id, amount))
        {
            // Movendo os itens de volta para o inventário do jogador
            ReturnValue ret = g_game.internalMoveItem(NULL, item->getParent(), this, INDEX_WHEREEVER, item, item->getItemCount(), 0);
            if (ret != RET_NOERROR)
            {
                std::cout << "[Error] Failed to move item back to player inventory." << std::endl;
                return false;
            }

            // Deletando a oferta do mercado
            query.str("");
        ...
        {
            std::cout << "[Warning] Failed to create item for market offer." << std::endl;
            return false;
        }
    }

    std::cout << "[Error] Market offer not found." << std::endl;
    return false;
}
...
{
    // Calcula o preço total com base na quantidade de itens comprados
    uint64_t totalPrice = price * countSelectedScrollBar;

    // Verifica se o comprador tem pontos suficientes para o preço total
    bool hasPoints = IOLoginData::getInstance()->getPoints(this, getAccount(), totalPrice);
    if (!hasPoints) {
        return false;
    }
    ...
    if ((result = db->storeQuery(query.str())))
    {
        ...
        else if (item_name.find("[E]") != std::string::npos) raridadeLevel = 3;
        else if (item_name.find("[L]") != std::string::npos) raridadeLevel = 4;
        else if (item_name.find("[M]") != std::string::npos) raridadeLevel = 5;
        ...
        if (Item* item = Item::CreateItem(item_id, countSelectedScrollBar))
        {
			...
			// Configura��o de atributos e nome com base na raridade
            if (raridadeLevel >= 1 && raridadeLevel <= 5)
            {
            ...
            if (player) {
                // O vendedor recebe o valor total (preço unitário * quantidade comprada)
                IOLoginData::getInstance()->addPoints(player, player->getAccount(), totalPrice);
                IOLoginData::getInstance()->savePlayer(player);
            }

            // O comprador paga o valor total
            IOLoginData::getInstance()->removerPoints(this, getAccount(), totalPrice);
        }

...
uint32_t CreatureEvent::executeThrow(Player* player, Item* item, const Position& fromPosition, const Position& toPosition, bool fromPlayer, bool toPlayer)
{
	//onThrow(cid, item, fromPosition, toPosition, fromPlayer, toPlayer)
	if(m_interface->reserveEnv())
	{
		ScriptEnviroment* env = m_interface->getEnv();
		if(m_scripted == EVENT_SCRIPT_BUFFER)
		{
			...
			scriptstream << "local fromPlayer = " << (fromPlayer ? "true" : "false") << std::endl;
			scriptstream << "local toPlayer = " << (toPlayer ? "true" : "false") << std::endl;
		...
		else
		{
			...
			lua_pushboolean(L, fromPlayer);
			lua_pushboolean(L, toPlayer);
